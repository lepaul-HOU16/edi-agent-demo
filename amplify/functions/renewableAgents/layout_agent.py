...
try:
    ...
    from strands_agents_tools import think, use_agent
...
@tool
def layout_agent(region_name="us-west-2", model_id="us.anthropic.claude-3-7-sonnet-20250219-v1:0", query="No prompt found in input, please guide customer to create a json payload with prompt key", bedrock_client=None) -> str:
    """
    ...
    try:
        ...
        if bedrock_client is not None:
            logger.info("♻️  Using pooled Bedrock client for layout agent")
            ...
            logger.info("🔌 Creating new Bedrock client for layout agent")
...
# Removed @app.entrypoint since we're not using bedrock_agentcore
# The lambda_handler.py will call layout_agent() directly
