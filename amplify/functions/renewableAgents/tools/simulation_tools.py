...
# Lazy load weasyprint (optional dependency for PDF generation)
_weasyprint = None

def get_weasyprint():
    """Lazy load weasyprint only when needed for PDF generation"""
    global _weasyprint
    if _weasyprint is None:
        try:
            import weasyprint
            _weasyprint = weasyprint
            logger.info("Loaded weasyprint for PDF generation")
        except ImportError:
            logger.warning("weasyprint not available - PDF generation will be skipped")
            _weasyprint = False
    return _weasyprint if _weasyprint is not False else None
...
from lazy_imports import get_pywake, get_matplotlib, get_matplotlib_pyplot
...
def run_simplified_flow_map_simulation(simplified_conditions, turbines_geojson, turbine_model=None):
    ...
    try:
        # Lazy load PyWake when needed
        py_wake = get_pywake()
        IEA37SimpleBastankhahGaussian = py_wake.IEA37SimpleBastankhahGaussian
        IEA37_WindTurbines = py_wake.examples.data.iea37._iea37.IEA37_WindTurbines
        WindTurbine = py_wake.wind_turbines.WindTurbine
        PowerCtTabular = py_wake.wind_turbines.power_ct_functions.PowerCtTabular
        UniformWeibullSite = py_wake.site.UniformWeibullSite
...
def generate_charts(project_id: str, simulation_id: str) -> Dict:
    ...
    try:
        # Lazy load matplotlib when needed for chart generation
...
def create_executive_report(wind_conditions: Dict, simulation_result: Dict, summary: str) -> str:
    ...
    # Create PDF from HTML (if weasyprint available)
    try:
        ...
        weasyprint_module = get_weasyprint()
        if weasyprint_module:
            # Generate PDF to temporary file first
            with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as temp_file:
                weasyprint_module.HTML(string=html_content).write_pdf(temp_file.name)
            ...
            logger.warning(f"Skipping PDF generation - weasyprint not available. Saving HTML instead.")
            # Save HTML version instead
            html_filename = filename.replace('.pdf', '.html')
            save_file_with_storage(html_content, project_id, html_filename, "text", "simulation_agent")
