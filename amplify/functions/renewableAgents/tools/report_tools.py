...
def create_pdf_report_with_images(project_id: str, markdown_content: str, image_filenames: List[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Create PDF using temporary file (if weasyprint available)
        weasyprint_module = get_weasyprint()
        if weasyprint_module:
            with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as temp_file:
                weasyprint_module.HTML(string=styled_html).write_pdf(temp_file.name)
            ...
            logger.warning(f"Skipping PDF generation for {project_id} - weasyprint not available")
