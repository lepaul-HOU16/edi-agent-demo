...
from lazy_imports import get_geopandas, get_matplotlib, get_matplotlib_pyplot
...
def create_geodataframe_from_features(features):
    """Helper function to create GeoDataFrame with lazy loading"""
    gpd = get_geopandas()
    return create_geodataframe_from_features(features)
...
def is_point_in_boundaries(lat: float, lon: float, boundaries_gdf) -> bool:
...
def create_layout_map(
    ...
    try:
        ...
        # Lazy load matplotlib when needed for visualization
        ...
        boundaries = load_boundaries_geojson(project_id)
        if boundaries and boundaries.get('features'):
            gdf = create_geodataframe_from_features(boundaries['features'])
            gdf.crs = 'EPSG:4326'
