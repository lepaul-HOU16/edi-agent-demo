...
export class EDIcraftMCPClient {
     ...
     */
    async invokeAgentWithRetry(message, maxRetries = 3) {
        let lastError = null;
    ...
    async processAgentResponse(response) {
        ...
        const thoughtSteps = [];
        let stepCounter = 0;
        const seenStepTypes = new Set(); // Track unique step types to avoid duplicates
        try {
     ...
     */
    extractThoughtStepFromTrace(trace, stepId) {
        try {
