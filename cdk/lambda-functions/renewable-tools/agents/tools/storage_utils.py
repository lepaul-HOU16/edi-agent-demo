...
def get_s3_config():
    """Get S3 configuration from environment variables or SSM parameters"""
    try:
        # PRIORITY 1: Check environment variables first (for Lambda deployment)
        bucket_name = os.environ.get('RENEWABLE_S3_BUCKET')
        if bucket_name:
            logger.debug(f"Using S3 bucket from environment: {bucket_name}")
            return {
                'use_s3': True,
                'bucket_name': bucket_name
            }
        
        # PRIORITY 2: Fall back to SSM parameters
        ...
        # Don't raise - return disabled config
        return {'use_s3': False, 'bucket_name': None}
