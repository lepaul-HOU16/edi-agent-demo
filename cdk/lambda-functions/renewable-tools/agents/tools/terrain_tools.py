...
# Heavy dependencies - import only when needed
try:
    import folium
...
try:
    ...
    # Fallback if storage_utils not available
    def save_file_with_storage(*args, **kwargs):
        logging.warning("storage_utils not available, skipping file save")
        return None
...
def query_overpass(lat, lon, radius_km=5, setback_distance_m=100):
    ...
    # CRITICAL FIX: Use multiple Overpass API endpoints with retry logic
    # The main overpass-api.de is often overloaded and times out
    overpass_endpoints = [
        "https://overpass.kumi.systems/api/interpreter",  # Alternative endpoint (more reliable)
        "https://overpass-api.de/api/interpreter",        # Original endpoint (backup)
        "https://overpass.openstreetmap.ru/api/interpreter"  # Russian mirror (backup)
    ]
    ...
    for endpoint_url in overpass_endpoints:
        for attempt in range(2):  # 2 attempts per endpoint
            try:
                logger.debug(f"Attempting Overpass API call to {endpoint_url} (attempt {attempt + 1})")
                response = requests.post(endpoint_url, data=query, timeout=25)
                
                if response.status_code == 200:
                    logger.debug(f"✅ Overpass API success with {endpoint_url}")
                    break
                elif response.status_code == 429 or response.status_code == 504:
                    # Rate limited or timeout - try next endpoint
                    logger.warning(f"Overpass API {endpoint_url} returned {response.status_code}, trying next endpoint")
                    last_error = f"{response.status_code}: {response.text[:200]}"
                    continue
                else:
                    logger.error(f"Overpass API error from {endpoint_url}: {response.status_code}")
                    last_error = f"{response.status_code}: {response.text[:200]}"
                    continue
                    
            except requests.exceptions.Timeout:
                logger.warning(f"Timeout calling {endpoint_url}, trying next endpoint")
                last_error = f"Timeout from {endpoint_url}"
                continue
            except Exception as e:
                logger.warning(f"Error calling {endpoint_url}: {str(e)}")
                last_error = str(e)
                continue
    ...
    # CRITICAL FIX: Don't merge features - user expects to see all individual features
    # The simplify_geojson_union function was reducing 150+ features to just 4-6 merged features
    # This breaks the user's expectation of seeing all terrain features on the map
    # Instead, just simplify geometries without merging
    geojson_data = simplify_geometries_only(geojson_data)
    logger.debug(f"After geometry simplification: {len(geojson_data.get('features', []))} features")
...
def filter_by_radius(geojson_data, center_lat, center_lon, radius_km):
    ...
    # If geopandas is not available, use simple bounding box filter
    if not VISUALIZATION_AVAILABLE:
        logger.warning("Geopandas not available, using simple bounding box filter")
        ...
        for feature in geojson_data.get('features', []):
            ...
            if coords:
                ...
                if geom.get('type') == 'Point':
                    lon, lat = coords
                    if (abs(lat - center_lat) <= radius_deg and 
                        abs(lon - center_lon) <= radius_deg):
                        in_bounds = True
    ...
    # Use geopandas for accurate filtering
...
def apply_setback(geojson_data, setback_distance_m):
    ...
    # If geopandas not available, skip setback (return original data)
    if not VISUALIZATION_AVAILABLE:
        logger.warning(f"Geopandas not available, skipping {setback_distance_m}m setback")
        return geojson_data
...
def simplify_geometries_only(geojson_data):
    """Simplify geometries without merging features - preserves individual feature count"""
    
    # If geopandas not available, return original data
    if not VISUALIZATION_AVAILABLE:
    ...
    try:
        ...
        # Light simplification (tolerance ~10m) to reduce coordinate count
        # This reduces file size without merging features
        gdf['geometry'] = gdf.geometry.simplify(tolerance=0.0001, preserve_topology=True)
        
        # Remove tiny features (less than ~100 sq meters) that are likely noise
        gdf = gdf[gdf.geometry.area > 0.000001]
        
        # Convert back to GeoJSON - preserves all individual features
        return json.loads(gdf.to_json())
    except Exception as e:
        logger.error(f"Geometry simplification failed: {e}")
        return geojson_data

def simplify_geojson_union(geojson_data):
    """Reduce GeoJSON size by merging overlapping/adjacent polygons by type
    
    WARNING: This function merges features and reduces feature count dramatically.
    Use simplify_geometries_only() instead if you want to preserve individual features.
    """
...
def save_analysis_results(geojson_data, latitude, longitude, project_id):
    ...
    try:
        ...
        # Skip visualization if libraries not available
        if not VISUALIZATION_AVAILABLE:
            logger.warning("Visualization libraries not available, skipping map/image generation")
            return {
                'geojson_saved': True,
                'map_saved': False,
                'image_saved': False,
...
def get_unbuildable_areas(latitude: float, longitude: float, project_id: str, 
    ...
    try:
        ...
        # CRITICAL FIX: Don't return full GeoJSON (can be 6MB+, exceeds Lambda limit)
        # Instead, return only summary with S3 references
        feature_count = len(terrain_data.get('features', []))
        ...
        return {
            ...
            "geojson": {
                "type": "FeatureCollection",
                "features": terrain_data.get('features', [])[:5]  # Return only first 5 features as preview
            },
            "s3_data": {
                "geojson_file": f"{project_id}/terrain_agent/boundaries.geojson",
                "map_file": f"{project_id}/terrain_agent/boundaries.html",
                "png_file": f"{project_id}/terrain_agent/boundaries.png"
            },
            "message": f"Analysis completed. Found {feature_count} exclusion zones. Full data saved to S3."
        }
