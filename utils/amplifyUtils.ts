...
export const getConfiguredAmplifyClient = () => {
  console.warn('getConfiguredAmplifyClient is deprecated - use REST API instead');
  throw new Error('Amplify client is no longer available - use REST API');
};

export const setAmplifyEnvVars = async () => {
  console.warn('setAmplifyEnvVars is deprecated - no longer needed with REST API');
  return {
    success: true
  };
};
