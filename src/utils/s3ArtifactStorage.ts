...
export const downloadArtifactFromS3 = async (
  ...
  try {
    ...
    // Get signed URL via REST API
    const { getFileUrl } = await import('@/lib/api/storage');
    const url = await getFileUrl(reference.key);
...
export const cleanupS3Artifacts = async (artifacts: any[]): Promise<void> => {
  ...
  const { deleteFile } = await import('@/lib/api/storage');
  
  for (const reference of s3References) {
    try {
      await deleteFile(reference.key);
