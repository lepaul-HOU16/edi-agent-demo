...
// Map functionality restored for development
import maplibregl, { 
  Map as MaplibreMap, 
  GeoJSONSource,
  MapMouseEvent,
...
function CatalogPage() {
  ...
  const handleChatSearch = useCallback(async (prompt: string) => {
    ...
    try {
      ...
      // Update the map with the new GeoJSON data using Amazon Location Service approach
      if (geoJsonData && mapRef.current && geoJsonData.type === 'FeatureCollection') {
        ...
        if (wellsSource) {
          ...
          // Add circle layer with data-driven styling for different well types
          mapRef.current.addLayer({
            ...
            paint: {
              ...
              'circle-color': [
                'case',
                ['==', ['get', 'type'], 'Production'], '#00AA00', // Green for Production
                ['==', ['get', 'type'], 'Exploration'], '#FF6600', // Orange for Exploration
                '#FF0000' // Red as default
              ],
...
`;
  ...
  useEffect(() => {
    // Initialize interactive map when selectedId is "seg-1"
    if (selectedId === "seg-1") {
      const mapContainer = document.getElementById("map");
      if (mapContainer && !mapRef.current) {
        
        // Create new map instance using Amazon Location Service
